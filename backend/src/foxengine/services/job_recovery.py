...
TASK_NAME_BY_JOB_TYPE: dict[str, str] = {
    ...
    "batch_tag": "foxengine_batch_tag",
    "batch_purge": "foxengine_purge_batch",
}
...
async def _defer_fox_job(job_type: str, job_id: UUID) -> None:
    ...
    defer_by_type = {
        ...
        "batch_tag": foxengine_batch_tag.defer_async,
        "batch_purge": foxengine_purge_batch.defer_async,
    }
...
async def retry_doing_procrastinate_for_running_fox_jobs() -> int:
    ...
    types_sql = ", ".join(f"'{t}'" for t in FOX_BACKGROUND_JOB_TYPES)
...
async def recover_fox_job(job_id: UUID) -> str:
    ...
    await _defer_fox_job(job_type, job_id)
    return "requeued"
